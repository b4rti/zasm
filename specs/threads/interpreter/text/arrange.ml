...
let memop name memop sz =
  value_type memop.ty ^ "." ^ memop_without_type name memop sz
...
let memoryatomicwaitop op =
  match op.sz with
  | None ->
    let sz = string_of_int (8 * size op.ty) in
    memop_without_type ("memory.atomic.wait" ^ sz) op (size op.ty)
...
let atomicloadop op =
  ...
  | Some sz ->
    memop ("atomic.load" ^ pack_size sz ^ "_u") op (packed_size sz)

let atomicstoreop op =
  match op.sz with
  | None -> memop "atomic.store" op (size op.ty)
  | Some sz ->
    memop ("atomic.store" ^ pack_size sz) op (packed_size sz)

let atomicrmwop op rmw_op =
  match op.sz with
  | None -> memop ("atomic.rmw." ^ rmwop rmw_op) op (size op.ty)
  | Some sz ->
    memop ("atomic.rmw" ^ pack_size sz ^ "." ^ rmwop rmw_op ^ "_u") op
      (packed_size sz)
...
let atomicrmwcmpxchgop op =
  ...
  | Some sz ->
    memop ("atomic.rmw" ^ pack_size sz ^ ".cmpxchg_u") op (packed_size sz)
